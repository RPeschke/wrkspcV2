...
#include <cstring>
#include <vector>
#include "KLM_lib/include/KLM_lib/KLM_Tree.hpp"
...
//friend std::ostream& operator<<(std::ostream& out, const KLM_Tree& evt){
//        out <<      evt.EvtNum      << " ";
//        out <<      evt.AddNum      << " ";
//        out <<      evt.WrAddNum    << " ";
//        out <<      evt.Wctime      << " ";
...
void CHECK_FOR_FEATURE_EXTRACTION(const std::vector<unsigned int>& buffer_uint, const int offset,const int count, KLM_Tree& evt){
...
bool CHECK_FOR_SAMPLE(const std::vector<unsigned int>& buffer_uint, const int offset,const int count,const int chanNum, const int window,  KLM_Tree& evt){
...
void extract_package_header(const std::vector<unsigned int>& buffer_uint, KLM_Tree& evt){
...
void extract_package_body(const std::vector<unsigned int>&  buffer_uint,  KLM_Tree& evt,std::ostream& out){
...
void processBuffer(const unsigned char* buff,const long size, const char* root_output,const char* trigBitOutfile,const int OutMode) {
    ...
    if (size<100) {
	  ...
	  TFile *_file0 = TFile::Open(root_output);
	  auto tree = new  KLM_Tree(tree);
